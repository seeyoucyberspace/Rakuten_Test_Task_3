...
class BasketActions {
    ...
    selectCountry() {
        cy.get(basketPageObject.getCountrySelectorLocator()).find('option:not([value="Choose..."])').then($options => {
            const randomIndex = Math.floor(Math.random() * $options.length);
            const randomOption = $options[randomIndex].value;
            cy.get(basketPageObject.getCountrySelectorLocator()).select(randomOption);
            cy.get(basketPageObject.getCountrySelectorLocator()).should('have.value', randomOption);
        });
    }

    // Select a random city (except the default "Choose...")
    selectCity() {
        cy.get(basketPageObject.getCitySelectorLocator()).find('option:not([value="Choose..."])').then($options => {
            const randomIndex = Math.floor(Math.random() * $options.length);
            const randomOption = $options[randomIndex].value;
            cy.get(basketPageObject.getCitySelectorLocator()).select(randomOption);
            cy.get(basketPageObject.getCitySelectorLocator()).should('have.value', randomOption);
        });
    }
}

export const basketActions = new BasketActions(basketPageObject);